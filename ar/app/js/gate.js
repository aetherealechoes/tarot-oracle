...
(function () {
  ...
  const PREFIX     = 'TAROT';
  const BUY_URL    = 'https://etherealjournals.gumroad.com/l/tarot-oracle-ar';
  const GUMROAD_PRODUCT_ID = '7UyqliUsUg1wJOYosw779g==';
  const TRIAL_DAYS = 7;
  ...
  function showEmailGate() {
    ...
    o.innerHTML =
      '<div class="tarot-gate-card">' +
        '<div class="tarot-gate-moon">🌙</div>' +
        '<h2 class="tarot-gate-title">' + t('emailTitle', 'ابدأ تجربتك المجانية') + '</h2>' +
        '<p class="tarot-gate-sub">' + t('emailSub', 'أدخل بريدك الإلكتروني لتفتح ٧ أيام مع العرّافة.') + '</p>' +
        '<input id="tarot-email-input" class="tarot-gate-input" type="email" placeholder="name@email.com" autocomplete="email" spellcheck="false" />' +
        '<button id="tarot-email-submit" class="tarot-gate-submit">' + t('emailStart', 'ابدأ تجربتي لمدة ٧ أيام ✦') + '</button>' +
    ...
    async function startTrial() {
      ...
      if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(email)) { err.textContent = t('emailInvalid', 'يرجى إدخال بريد إلكتروني صحيح.'); return; }
      btn.disabled = true; btn.textContent = t('gateValidating', 'جارٍ…');
      try {
        await fetch('https://formsubmit.co/ajax/' + EMAIL_TO, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Accept': 'application/json' },
          body: JSON.stringify({ email: email, _subject: 'New Tarot Oracle trial signup (AR)' })
        });
      } catch (e) {}
