...
const Social = {
  ...
  async generateShareableCard(reading, interpretation, callback) {
    ...
    ctx.fillText('Tarot Oracle', 300, 60);

    // Date
    ctx.fillStyle = 'rgba(200, 168, 78, 0.6)';
    ctx.font = '14px "Cormorant Garamond", serif';
    ctx.fillText(new Date().toLocaleDateString(), 300, 85);
    ...
    const cardName = (card) => {
      ...
      const nm = c.name;
      const base = (nm && typeof nm === 'object') ? (nm[lang] || nm.en) : (nm || c.id || '');
      return base + (card.isReversed ? ` (${I18N.t('reversed')})` : '');
    };

    let cursorY = 178;

    // Card names — 1 column up to 6 cards, 2 columns beyond
    if (N) {
      ctx.fillStyle = '#c8a84e';
      ctx.textAlign = 'center';
      if (N <= 6) {
        ...
        cards.forEach((card, i) => {
          ctx.fillText(`${i + 1}. ${cardName(card)}`, 300, cursorY);
          cursorY += 26;
        });
      } else {
        ctx.font = '14px "Cormorant Garamond", serif';
        ...
        for (let r = 0; r < half; r++) {
          ...
          ctx.fillText(`${li + 1}. ${cardName(cards[li])}`, 175, cursorY);
          if (cards[ri]) ctx.fillText(`${ri + 1}. ${cardName(cards[ri])}`, 425, cursorY);
          cursorY += 24;
        }
      }
      cursorY += 16;
    ...
    // Interpretation summary (smart truncation)
    if (interpretation) {
      const summary = interpretation.summary || '';
      const text = (typeof summary === 'object') ? (summary[lang] || summary.en || '') : summary;
  ...
  async shareReading(reading, interpretation) {
    ...
    // Phones / Safari: native OS share sheet with the image attached
    if (file && navigator.canShare && navigator.canShare({ files: [file] })) {
      try {
        ...
        // otherwise fall through to the modal
      }
    }

    // Desktop / unsupported: show the share modal
  ...
  _showShareModal(canvas, blob) {
    ...
    const mediaUrl = 'https://tarot.etherealechoes.com/en/images/oracle.png';
    ...
    overlay.innerHTML = `
      <div class="share-modal" role="dialog" aria-modal="true">
        <button class="share-modal-close" aria-label="${I18N.t('close') || 'Close'}">&times;</button>
    ...
    copyBtn.addEventListener('click', async () => {
      try {
        ...
        copyBtn.innerHTML = '&#10003; ' + (I18N.t('shareCopied') || 'Copied');
        setTimeout(() => { copyBtn.innerHTML = '&#10697; ' + I18N.t('shareCopy'); }, 1800);
      } catch (err) {
        this._downloadImage(canvas); // clipboard image unsupported → download
      }
    });
  },

  // Load an image, resolving to null on failure (so one bad image
  // never breaks card generation / sharing).
  _loadImage(src) {
