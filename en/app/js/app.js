...
const App = {
  ...
  init() {
    ...
    // If we have a saved language in profile, apply it — unless the user
    // arrived via an explicit ?lang= choice from the landing page.
    if (this.profile && this.profile.language && !I18N.urlLangOverride) {
      I18N.setLanguage(this.profile.language);
    }
  ...
  _langButtons(currentLang, actionAttr) {
    const action = actionAttr ? ` data-action="${actionAttr}"` : '';
    return this.LANGUAGES.map(l =>
      `<button class="lang-btn ${currentLang === l.code ? 'active' : ''}"${action} data-lang="${l.code}">${l.label}</button>`
    ).join('\n            ');
  },
  ...
  initOnboarding() {
    ...
    const rulerName = I18N.t(dailyEnergy.dayRuler);
    const elementName = I18N.t(dailyEnergy.elementOfDay.primary);
    ...
    const hookText = {
      en: `The ${moonName} ${moonEmoji} is overhead and ${rulerName} rules today, weaving ${elementName} energy through the hours. The Oracle sees what the day holds — but it needs your cosmic signature to speak to you directly.`,
      fr: `La ${moonName} ${moonEmoji} est au-dessus et ${rulerName} gouverne aujourd'hui, tissant l'énergie de ${elementName} à travers les heures. L'Oracle voit ce que le jour réserve — mais il a besoin de votre signature cosmique pour vous parler directement.`,
      es: `La ${moonName} ${moonEmoji} está sobre nosotros y ${rulerName} gobierna hoy, tejiendo energía de ${elementName} a través de las horas. El Oráculo ve lo que el día depara — pero necesita tu firma cósmica para hablarte directamente.`
    }[lang];

    screen.innerHTML = `
      <div class="onboarding-container">
        <h1 data-i18n="welcome">${I18N.t('welcome')}</h1>
        ...
        <div class="onboarding-oracle-img">
          <img src="images/oracle.png" alt="The Oracle" />
        </div>
        ...
        <div class="onboarding-form">
          ...
          <div class="onboarding-section">
            ...
            <div class="lang-buttons" id="onboarding-lang-buttons">
              ${this._langButtons(I18N.currentLang, '')}
            </div>
          </div>
        </div>
    ...
    const updatePreview = () => {
      ...
      const zodiac = Celestial.getZodiacSign(month, day);
      ...
      if (previewZodiac) previewZodiac.textContent = `${zodiac.emoji} ${I18N.t(zodiac.id)}`;
      if (previewLifepath) previewLifepath.textContent = lifePath;
      ...
      if (name.length > 1) {
        ...
        if (soulVal) soulVal.textContent = soul;
    ...
    // ── Rolling date picker ──────────────────────────────────
    const restoreDob = this._initDobRoller(updatePreview, lang);

    // Name input handler
    const nameInput = document.getElementById('name-input');
    if (nameInput) {
      nameInput.addEventListener('input', updatePreview);
      // Opening the mobile keyboard for the name field can reflow the page and
      // reset the roller's scroll (year snapping to the top). Re-align it.
      const realign = () => { if (restoreDob) setTimeout(restoreDob, 250); };
      nameInput.addEventListener('focus', realign);
      nameInput.addEventListener('blur', realign);
    }
  },

  // ── 4b. Rolling DOB Picker ────────────────────────────────
  _initDobRoller(onChange, lang, prefix = 'roller') {
    const ITEM_H = 44; // px height per item
    ...
    const monthVals = months.map((_, i) => i + 1);
    buildItems(monthCol, monthVals, (v, i) => months[i]);
    buildItems(yearCol, years, (v) => v);
    ...
    const rebuildDays = () => {
      ...
      const dayVals = [];
      for (let d = 1; d <= maxDay; d++) dayVals.push(d);
      buildItems(dayCol, dayVals, (v) => v);
      if (selectedDay && selectedDay > maxDay) {
        selectedDay = maxDay;
      }
      if (selectedDay) {
        scrollToValue(dayCol, selectedDay, false);
      }
    };
    ...
    const getSelectedValue = (col) => {
      ...
      if (idx >= 0 && idx < items.length) {
        return parseInt(items[idx].getAttribute('data-value'), 10);
      }
      return null;
    };

    const scrollToValue = (col, value, smooth) => {
      const items = col.querySelectorAll('.roller-item:not(.roller-pad)');
      for (let i = 0; i < items.length; i++) {
        if (parseInt(items[i].getAttribute('data-value'), 10) === value) {
    ...
    const syncHidden = () => {
      const dobInput = document.getElementById(hiddenId);
      if (selectedYear && selectedMonth && selectedDay && dobInput) {
        const mm = String(selectedMonth).padStart(2, '0');
        const dd = String(selectedDay).padStart(2, '0');
        dobInput.value = `${selectedYear}-${mm}-${dd}`;
        if (onChange) onChange();
      }
    };

    const highlightCenter = (col) => {
      const items = col.querySelectorAll('.roller-item:not(.roller-pad)');
      const scrollTop = col.scrollTop;
      const centerIdx = Math.round(scrollTop / ITEM_H);
      items.forEach((item, i) => {
        item.classList.toggle('roller-selected', i === centerIdx);
        const dist = Math.abs(i - centerIdx);
        if (dist === 0) { item.style.opacity = '1'; item.style.transform = 'scale(1)'; }
        else if (dist === 1) { item.style.opacity = '0.55'; item.style.transform = 'scale(0.92)'; }
        else { item.style.opacity = '0.25'; item.style.transform = 'scale(0.85)'; }
      });
    };

    const onScroll = (col, role) => {
      let ticking = false;
      ...
      col.addEventListener('scroll', () => {
        ...
        scrollTimer = setTimeout(() => {
          ...
          if (role === 'month') {
            selectedMonth = val;
            const maxDay = daysInMonth(selectedMonth, selectedYear);
            if (selectedDay && selectedDay > maxDay) selectedDay = maxDay;
            rebuildDays();
          ...
          // Commit the centered value of EVERY column — so a column the user
          // never scrolled (left at its default) still counts. Without this,
          // the Begin button stays disabled until all three are scrolled.
          const cm = getSelectedValue(monthCol); if (cm !== null) selectedMonth = cm;
          const cd = getSelectedValue(dayCol);   if (cd !== null) selectedDay   = cd;
          const cyr = getSelectedValue(yearCol); if (cyr !== null) selectedYear  = cyr;
          scrollToValue(col, val, true);
          syncHidden();
        }, 80);
      }, { passive: true });
    };

    onScroll(monthCol, 'month');
    onScroll(dayCol, 'day');
    onScroll(yearCol, 'year');
    ...
    // Open on a plausible birth date (1990 / June / 15) instead of the current
    // year, and commit it so the Begin button is usable from the start — the
    // user adjusts from a sensible starting point rather than seeing "2026".
    const DEF_MONTH = 6, DEF_DAY = 15, DEF_YEAR = 1990;
    setTimeout(() => {
      if (!document.body.contains(yearCol)) return;
      selectedMonth = DEF_MONTH;
      selectedYear = DEF_YEAR;
      rebuildDays();
      selectedDay = DEF_DAY;
      scrollToValue(monthCol, DEF_MONTH, false);
      scrollToValue(dayCol, DEF_DAY, false);
      scrollToValue(yearCol, DEF_YEAR, false);
      highlightCenter(monthCol); highlightCenter(dayCol); highlightCenter(yearCol);
      syncHidden();
    }, 60);

    // Re-align the roller to its selected values after a layout reflow
    // (a mobile keyboard opening for the name field can reset the columns'
    // scroll position, snapping the year back to the top / current year).
    const restoreScroll = () => {
      if (!document.body.contains(yearCol)) return;
      if (selectedMonth) scrollToValue(monthCol, selectedMonth, false);
      if (selectedDay)   scrollToValue(dayCol, selectedDay, false);
      if (selectedYear)  scrollToValue(yearCol, selectedYear, false);
      highlightCenter(monthCol); highlightCenter(dayCol); highlightCenter(yearCol);
    };
    let _dobResizeTimer = null;
    window.addEventListener('resize', () => {
      if (!document.body.contains(yearCol)) return; // roller no longer on screen
      clearTimeout(_dobResizeTimer);
      _dobResizeTimer = setTimeout(restoreScroll, 150);
    });
  ...
  showHome() {
    ...
    screen.innerHTML = `
      ...
      <select class="home-nav-dropdown" id="home-nav-dropdown">
        <option value="">${I18N.t('navigation') || 'Navigation'}</option>
        <option value="show-numerology">&#128290; ${I18N.t('numerologyReading')}</option>
        <option value="show-glossary">&#128218; ${I18N.t('glossary')}</option>
        <option value="show-journal">&#128216; ${I18N.t('journal')}</option>
        <option value="show-mirror">&#128302; ${I18N.t('mirror')}</option>
      </select>
      ...
      <section class="spread-section">
        ...
        <select class="spread-dropdown" id="spread-dropdown">
          <option value="">${I18N.t('selectSpread') || 'Select a spread...'}</option>
        </select>
    ...
    // Setup navigation dropdown listener
    const navDropdown = document.getElementById('home-nav-dropdown');
    if (navDropdown) {
    ...
    // Setup spread dropdown listener
    const spreadDropdown = document.getElementById('spread-dropdown');
    if (spreadDropdown) {
      spreadDropdown.addEventListener('change', (e) => {
        const spreadId = e.target.value;
        if (spreadId) {
          this.startReading(spreadId);
          e.target.value = ''; // Reset dropdown
        }
      });
    }
    ...
    if (this.profile && this.profile.dob) {
      ...
      personalDayHTML = `
        <div class="oracle-item">
          ...
          <span class="oracle-value">${personalDay} &mdash; ${meaning.theme[lang] || meaning.theme.en}</span>
        </div>
      `;
      ...
      if (this.profile.fullName) {
        ...
        <div class="oracle-item">
          ...
          <span class="oracle-value">${expr} &mdash; ${exprMeaning?.[lang] || exprMeaning?.en || ''}</span>
        </div>`;
      }
    }

    // Build the synthesis — this IS the oracle's value proposition
    const synthesis = this._buildDailySynthesis(dailyEnergy, lang);

    panel.innerHTML = `
      <div class="oracle-synthesis">${synthesis}</div>
  ...
  // ── 7b. Build Daily Synthesis Narrative ──────────────────────
  // Paragraph 1: personal — how today's energy meets the user + actionable directive
  // Paragraph 2: cosmic — moon phase counsel (what to lean into / hold back)
  _buildDailySynthesis(dailyEnergy, lang) {
    const moonPhase = dailyEnergy.moonPhase;
    const moonInfluence = dailyEnergy.moonInfluence;
    const elementOfDay = dailyEnergy.elementOfDay;

    const moonName = I18N.t(moonPhase.name);
    const elementName = I18N.t(elementOfDay.primary);

    // Moon counsel (used in cosmic paragraph)
    const mTheme = (typeof moonInfluence.theme === 'object') ? (moonInfluence.theme[lang] || moonInfluence.theme.en) : moonInfluence.theme;

    // Personal paragraph — sign + element harmony + numerology directive
    let personalLine = '';
    if (this.profile && this.profile.dob) {
      const { month, day } = this.profile.dob;
      const zodiac = this.profile.zodiac || Celestial.getZodiacSign(month, day);
      const sign = I18N.t(zodiac.id);
      const userElement = zodiac.element;
      const harmony = Celestial.getElementalHarmony(userElement, elementOfDay.primary);

      const personalDay = Numerology.getPersonalDay(month, day);
      const pdMeaning = Numerology.getMeaning(personalDay);
      const pdTheme = pdMeaning?.theme?.[lang] || pdMeaning?.theme?.en || '';
      // Pull the first sentence of the advice as the actual directive
      const pdAdviceFull = pdMeaning?.advice?.[lang] || pdMeaning?.advice?.en || '';
      const pdAdvice = pdAdviceFull.split(/(?<=[.!?])\s+/)[0] || pdAdviceFull;

      if (harmony === 'amplified') {
        personalLine = {
          en: `As a ${sign}, today's ${elementName} energy mirrors your own nature — the universe is speaking your language. Your Personal Day ${personalDay} — ${pdTheme.toLowerCase()} — sharpens this: ${pdAdvice}`,
          fr: `En tant que ${sign}, l'énergie de ${elementName} reflète votre propre nature — l'univers parle votre langage. Votre jour personnel ${personalDay} — ${pdTheme.toLowerCase()} — précise cela : ${pdAdvice}`,
          es: `Como ${sign}, la energía de ${elementName} refleja tu propia naturaleza — el universo habla tu idioma. Tu día personal ${personalDay} — ${pdTheme.toLowerCase()} — lo concreta: ${pdAdvice}`
        }[lang];
      } else if (harmony === 'challenging') {
        personalLine = {
          en: `As a ${sign}, today's ${elementName} energy pushes you outside your comfort zone — growth often hides in this friction. Your Personal Day ${personalDay} — ${pdTheme.toLowerCase()} — offers the way through: ${pdAdvice}`,
          fr: `En tant que ${sign}, l'énergie de ${elementName} vous pousse hors de votre zone de confort — la croissance se cache dans cette friction. Votre jour personnel ${personalDay} — ${pdTheme.toLowerCase()} — offre le passage : ${pdAdvice}`,
          es: `Como ${sign}, la energía de ${elementName} te empuja fuera de tu zona de confort — el crecimiento se esconde en esta fricción. Tu día personal ${personalDay} — ${pdTheme.toLowerCase()} — te muestra el camino: ${pdAdvice}`
        }[lang];
      } else {
        personalLine = {
          en: `As a ${sign}, today's ${elementName} energy brings a supportive undercurrent to your day. Your Personal Day ${personalDay} — ${pdTheme.toLowerCase()} — shapes how to use it: ${pdAdvice}`,
          fr: `En tant que ${sign}, l'énergie de ${elementName} apporte un courant favorable à votre journée. Votre jour personnel ${personalDay} — ${pdTheme.toLowerCase()} — oriente son usage : ${pdAdvice}`,
          es: `Como ${sign}, la energía de ${elementName} trae una corriente favorable a tu día. Tu día personal ${personalDay} — ${pdTheme.toLowerCase()} — orienta su uso: ${pdAdvice}`
        }[lang];
      }
    }

    // Cosmic paragraph — moon phase counsel only (no planetary ruler, avoids redundancy)
    const cosmicLine = {
      en: `The ${moonName} adds a quality of ${mTheme} — let it temper the pace of your day.`,
      fr: `La ${moonName} ajoute une qualité de ${mTheme} — laissez-la tempérer le rythme de votre journée.`,
      es: `La ${moonName} agrega una cualidad de ${mTheme} — deja que modere el ritmo de tu día.`
    }[lang];

    return personalLine
      ? `<p class="synthesis-personal">${personalLine}</p><p class="synthesis-cosmic">${cosmicLine}</p>`
      : `<p class="synthesis-cosmic">${cosmicLine}</p>`;
  },
  ...
  renderSpreadOptions() {
    ...
    dropdown.innerHTML = `<option value="">${I18N.t('selectSpread') || 'Select a spread...'}</option>`;
    spreads.forEach((spread) => {
      const option = document.createElement('option');
      option.value = spread.id;
      option.textContent = `${spread.icon} ${I18N.t(spread.nameKey)}`;
  ...
  renderOthersPanel() {
    ...
    // Saved people chips
    people.forEach((p) => {
      const zodiac = Celestial.getZodiacSign(p.dob.month, p.dob.day);
      html += `
      <button class="person-chip ${activeId === p.id ? 'active' : ''}" data-action="select-person" data-person-id="${p.id}">
        <span class="chip-icon">${zodiac.emoji}</span>
        <span class="chip-name">${p.name}</span>
        <span class="chip-sign">${I18N.t(zodiac.id)}</span>
    ...
    if (activeId) {
      ...
      if (person) {
        const zodiac = Celestial.getZodiacSign(person.dob.month, person.dob.day);
        ...
        <div class="reading-for-banner">
          <span>${I18N.t('readingFor')} <strong>${person.name}</strong> — ${zodiac.emoji} ${I18N.t(zodiac.id)}, ${I18N.t('lifePathNumber')} ${lifePath}</span>
        </div>`;
      }
    }
  ...
  startReading(spreadId) {
    ...
    if (this.readingForId && this.profile && this.profile.people) {
      const person = this.profile.people.find(p => p.id === this.readingForId);
      if (person) {
        const z = Celestial.getZodiacSign(person.dob.month, person.dob.day);
        readingForHTML = `<div class="reading-for-banner" style="margin-bottom:var(--space-md)">${I18N.t('readingFor')} <strong>${person.name}</strong> ${z.emoji}</div>`;
      }
    }
    ...
    screen.innerHTML = `
      ...
      <div class="oracle-shuffle-scene" id="oracle-shuffle">
        <img src="images/oracle.png" alt="Oracle" class="oracle-image" id="oracle-image"/>
        ${I18N.currentLang !== 'en' ? `<p class="oracle-greeting" id="oracle-greeting-text">${I18N.t('oracleGreeting')}</p>` : ''}
      </div>
      ` : ''}
    ...
    interpArea.innerHTML = `
      <div class="interp-inline" role="region" aria-label="${interpreted.cardName}">
        ...
        <div class="interp-meaning">
          <p>${interpreted.baseMeaning}</p>
        </div>
        ${interpreted.personalizedNote ? `
        <div class="interp-personalized">
          <p>${interpreted.personalizedNote}</p>
        </div>
        ` : ''}
      </div>
    `;
  ...
  showReadingSummary(fullInterpretation) {
    ...
    const summary = _sanitize(fullInterpretation.summary);
    const dailyContext = _sanitize(fullInterpretation.dailyContext);
    ...
    if (this.currentReading.spread.id === 'yesNo' && fullInterpretation.cards.length === 1) {
      ...
      const yesNoResult = Interpreter.interpretYesNo(dealt.card, dealt.isReversed, I18N.currentLang);
      yesNoHTML = `
        <div class="yes-no-result">
          <span class="yes-no-answer yes-no-${yesNoResult.answer}">${yesNoResult.answer.toUpperCase()}</span>
    ...
    screen.innerHTML = `
      ...
      <div class="settings-content">
        <div class="settings-group">
          <label data-i18n="language">${I18N.t('language')}</label>
          <div class="lang-buttons" id="settings-lang-buttons">
            ${this._langButtons(currentLang, 'set-language')}
          </div>
        </div>
  ...
  showJournal(viewDate) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      calendarHTML += `<span class="${classes.join(' ')}" data-action="show-day-readings" data-date="${dateKey}">
        ${d}
        ${hasReading ? `<span class="cal-dot">${hasReading > 1 ? hasReading : ''}</span>` : ''}
      </span>`;
    }
    calendarHTML += '</div>';
    ...
    if (dayEntries.length > 0) {
      const dateDisplay = new Date(selectedKey + 'T12:00:00').toLocaleDateString(lang === 'fr' ? 'fr-FR' : lang === 'es' ? 'es-ES' : 'en-US', { weekday: 'long', month: 'long', day: 'numeric' });
      ...
      dayEntries.forEach(entry => {
        ...
        entriesHTML += `
          <div class="journal-entry">
            ...
            ${entry.summary ? `<p class="journal-summary">${entry.summary.substring(0, 200)}${entry.summary.length > 200 ? '...' : ''}</p>` : ''}
          </div>`;
      });
    } else {
    ...
    container.innerHTML = `
      ...
      <div class="calendar-container">
        <div class="cal-nav">
          ...
          <span class="cal-month-year">${mNames[month]} ${year}</span>
  ...
  showMirror() {
    ...
    const elementHTML = analytics.elementStats.map(e => `
      <div class="mirror-row">
        ...
        <span class="mirror-count">${e.pct}%</span>
      </div>
    `).join('');
    ...
    const suitHTML = analytics.suitStats.map(s => `
      <div class="mirror-row">
        ...
        <span class="mirror-count">${s.pct}%</span>
      </div>
    `).join('');
    ...
    const spreadHTML = analytics.spreadStats.map(s => `
      <div class="mirror-row">
        ...
        <span class="mirror-count">${s.count}x</span>
      </div>
    `).join('');
    ...
    container.innerHTML = `
      ...
      <div class="mirror-stats-row">
        <div class="mirror-stat-card">
          <span class="stat-number">${analytics.totalReadings}</span>
          <span class="stat-label">${I18N.t('totalReadings')}</span>
        </div>
        <div class="mirror-stat-card">
          <span class="stat-number">${analytics.streak}</span>
          <span class="stat-label">${I18N.t('currentStreak')}</span>
        </div>
        <div class="mirror-stat-card">
          <span class="stat-number">${analytics.uprightPct}%</span>
      ...
      <div class="mirror-section">
        ...
        <div class="upright-reversed-bar">
          <div class="ur-upright" style="width:${analytics.uprightPct}%">
            <span>${I18N.t('upright')} ${analytics.uprightPct}%</span>
          </div>
          <div class="ur-reversed" style="width:${analytics.reversedPct}%">
            <span>${I18N.t('reversed')} ${analytics.reversedPct}%</span>
          </div>
        </div>
      </div>
  ...
  // ── 14c-2. Elemental Pattern (neutral pattern-reading) ───────
  // Gate: needs >= MIN_READINGS completed readings before any text fires.
  // Prominent = single highest element at >= 40% of elemental cards.
  // Scarce    = single lowest element at <= 10%.
  // Shows at most ONE prominent + ONE scarce (different elements).
  ...
  _buildElementalPattern(analytics) {
    ...
    if (blocks.length === 0) return ''; // gated met, but nothing notable
  ...
  showGlossaryCard(cardId) {
    ...
    // Scroll to very top of page to show card image first
    setTimeout(() => {
  ...
  showNumerology() {
        ...
        return `
          <div class="num-card num-accent-${accent}">
            <div class="num-card-header">
              <span class="num-card-number">${number}</span>
      ...
      return `
        <div class="num-card num-accent-${accent}">
          ...
          <div class="num-card-body">
            ...
            ${shadow ? `<p class="num-shadow"><span class="shadow-label">${{en:'Watch for:',fr:'Attention :',es:'Cuidado:'}[lang]||'Watch for:'}</span> ${shadow}</p>` : ''}
          </div>
        </div>`;
    };
    ...
    if (fullName && numProfile.expression) {
      const exprText = numProfile.expressionMeaning?.[lang] || numProfile.expressionMeaning?.en || '';
      const soulText = numProfile.soulUrgeMeaning?.[lang] || numProfile.soulUrgeMeaning?.en || '';
      const persText = numProfile.personalityMeaning?.[lang] || numProfile.personalityMeaning?.en || '';
  ...
  // Mobile-friendly reset confirmation modal
  _showResetConfirmModal() {
    ...
    overlay.innerHTML = `
      <div class="confirm-modal" role="dialog" aria-modal="true">
        <h3 class="confirm-modal-title">${I18N.t('resetProfile')}</h3>
        <p class="confirm-modal-message">${I18N.t('resetConfirm')}</p>
        <div class="confirm-modal-actions">
          <button class="btn-cancel" id="reset-cancel">${I18N.t('cancel') || 'Cancel'}</button>
          <button class="btn-danger" id="reset-confirm">${I18N.t('resetProfile')}</button>
        </div>
      </div>`;
  ...
  _setupEventDelegation() {
      ...
      switch (action) {
        ...
        case 'share-reading': {
          ...
          if (!this.currentReading) {
            console.error('Share failed: No current reading');
            break;
          }
          if (!this.currentInterpretation) {
            console.error('Share failed: No interpretation generated');
            break;
          }
          console.log('Calling Social.shareReading...');
        ...
        case 'show-add-person': {
          ...
          if (form) {
            ...
            if (!form.classList.contains('hidden')) {
              ...
              const apLang = I18N.currentLang || 'en';
              this._initDobRoller(null, apLang, 'ap-roller');
            }
          }
          break;
        }
        ...
        case 'set-language': {
          ...
          if (lang) {
            ...
            const parentGroup = target.closest('.lang-buttons');
            if (parentGroup) {
              parentGroup.querySelectorAll('.lang-btn').forEach((btn) => {
                btn.classList.toggle('active', btn.getAttribute('data-lang') === lang);
              });
            }

            // Re-render appropriate screen content
            const activeScreen = document.querySelector('.screen.active');
            if (activeScreen) {
              if (activeScreen.id === 'screen-settings') {
                this.showSettings();
              } else if (activeScreen.id === 'screen-onboarding') {
                this.initOnboarding();
              } else if (activeScreen.id === 'screen-home') {
                this.showHome();
              }
            }
    ...
    // Handle home navigation dropdown (mobile)
    const navDropdown = document.getElementById('home-nav-dropdown');
    if (navDropdown) {
      navDropdown.addEventListener('change', (e) => {
        const action = e.target.value;
    ...
    document.body.addEventListener('click', (e) => {
      ...
      const langBtn = e.target.closest('#onboarding-lang-buttons .lang-btn');
      if (langBtn) {
        ...
        if (lang) {
          ...
          // Update active state
          const parentGroup = langBtn.closest('.lang-buttons');
          if (parentGroup) {
            parentGroup.querySelectorAll('.lang-btn').forEach((btn) => {
              btn.classList.toggle('active', btn.getAttribute('data-lang') === lang);
            });
          ...
          // Re-render onboarding with new language
          this.initOnboarding();
          this.updateUILanguage();
        }
      }
